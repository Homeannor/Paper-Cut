...
public class CameraSwitcher : MonoBehaviour
{
    ...
    private int currentCameraIndex = 1; // Index to track the current active camera
    ...
    public float rotateLerppeed = 1f;
    ...
    public void UpdateCameraList()
    {
        ...
        if (plane != null)
        {
            Camera planeCamera = GameObject.Find("PlaneCamera").GetComponent<Camera>(); // Include inactive objects
            if (planeCamera != null)
            {
    ...
    public void movePlaneCamera()
    {
        Vector3 targetPosition = planeTransform.position - planeTransform.up * 5;
        planeCameraTransform.position = Vector3.Lerp(planeCameraTransform.position, targetPosition, moveLerpSpeed * Time.deltaTime);

        Quaternion targetAngle = planeTransform.rotation * Quaternion.Euler(0, 0, 180);
        planeCameraTransform.rotation = Quaternion.Lerp(planeCameraTransform.rotation, targetAngle, rotateLerppeed * Time.deltaTime);
        // Debug.Log("Camera Position: " + transform.position + " // Camera Rotation: " + transform.rotation);
    }
}
